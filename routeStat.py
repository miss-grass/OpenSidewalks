...
def main():
    ...
    alleviations = np.load('OpenSidewalks/new_edges_data/elevations.npy')
    ...
    for i in range(start_num, end_num):
        ...
        # start point
        result[i][0] = str(xs) + "," + str(ys)
        # end point
        result[i][1] = str(xe) + "," + str(ye)
        # set of points on the route
        result[i][2] = routes[i]
        # set of slope for each edge
        result[i][9] = alleviations[i]
        ...
        if severe > 0:
            result[i][7] = float(severe) / float(dist)

        # severe issue number / meter
        if minor > 0:
            result[i][8] = float(minor) / float(dist)

        print(result[i])

    filename = "bigTable/output-" + str(start_num) + "-" + str(end_num - 1) + ".npy"
